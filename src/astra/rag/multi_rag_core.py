...
class MultiRAGRetriever:
    ...
    async def retrieve(
        ...
        """
        from astra.rag.modules.events import get_event_bus, RetrievalEvent, AnswerabilitySignal
        start_time = time()
        ...
        for category in categories:
            ...
            # Calculate metrics
            cat_latency = (time() - cat_start_time) * 1000
            final_results = optimized_results[:cat_config["retrieval"]["top_k"]]
            candidates[category] = final_results
            ...
            event = RetrievalEvent(
                ...
                metadata={
                    ...
                    "mix_alpha": cat_config["retrieval"]["mix_alpha"]
                }
            )
            ...
            if final_results:
                diversity = len({r.chunk.doc_id for r in final_results}) / len(final_results)
                margin = final_results[0].score - final_results[min(1, len(final_results)-1)].score
                answerable = (diversity >= 0.35 and margin >= 0.08)
                ...
                signal = AnswerabilitySignal(
                    ...
                    cache_hit=False,  # Cache handled by memory bridge
                    latency_ms=cat_latency,
                    metadata={
                        "category": category,
        ...
        total_latency = (time() - start_time) * 1000
