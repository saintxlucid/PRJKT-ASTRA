...
def main():
    ...
    # Debounce watcher: batch files for 1s
    import threading
    batch = []
    def process_batch():
        files = list(batch)
        ...
        for p in files:
            ...
            # Batch embedding
            batch_embed = cfg.get('system', {}).get('policies', {}).get('ingestion', {}).get('batch_embed', 64)
            for i in range(0, len(chunks), batch_embed):
                chunk_batch = chunks[i:i+batch_embed]
                vecs = embed_dense(chunk_batch, dims=cfg['memory']['vector']['dims'])
                ts = int(time.time())
                payloads = []
                for j, ch in enumerate(chunk_batch):
                    payloads.append({
                        'doc_id': doc_id,
                        'chunk_id': f"{i+j:05d}",
                        'text': ch,
    ...
    def debounce():
        ...
        timer = threading.Timer(1.0, process_batch)
        timer.start()
