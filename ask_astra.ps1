...
try {
    ...
    Write-Host "      [FAIL] Failed to create conversation" -ForegroundColor Red
...
Write-Host ""
Write-Host "[2/3] Sending message to ASTRA..." -ForegroundColor Yellow
...
- Docs: PDF ingest to chunk to sentence-transformers embeddings to Qdrant search; keyword fallback if vectors are down.
...
try {
    ...
    Write-Host "      [FAIL] Failed to get response" -ForegroundColor Red
    Write-Host "      Error: $($_.Exception.Message)" -ForegroundColor Red
...
Write-Host ""
Write-Host "[3/3] Processing response..." -ForegroundColor Yellow
...
Write-Host "[*] Full response saved to: ASTRA_PRODUCTION_READINESS_ASSESSMENT.txt" -ForegroundColor Green
